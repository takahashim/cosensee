...
RSpec.describe Cosensee::LineParser do
  ...
  describe '#parse_codeblock' do
    it 'parse codeblock segments' do
      expect(parser.parse_codeblock('')).to eq ['', '']
      expect(parser.parse_codeblock('code:')).to eq ['', 'code:']
      expect(parser.parse_codeblock('code:abc')).to eq ['abc', '']
      expect(parser.parse_codeblock('code:abc def')).to eq ['abc def', '']
      expect(parser.parse_codeblock('code:abc[] `de`f')).to eq ['abc[] `de`f', '']
    end
  end
