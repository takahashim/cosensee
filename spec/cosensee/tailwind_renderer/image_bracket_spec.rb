  ...
  describe '#render' do
    context 'when link is nil' do
      it 'convert ImageBracket to HTML' do
        ...
        expect(Cosensee::TailwindRenderer::ImageBracket.new(content:, project: nil).render).to eq %(<span><img src="https://example.com/example.jpg" loading="lazy" class="max-w-max max-h-80"></span>)
      end
    end
    ...
    context 'when link is not nil' do
      it 'convert ImageBracket to HTML' do
        ...
        expect(Cosensee::TailwindRenderer::ImageBracket.new(content:, project: nil).render).to eq %(<span><a href="http://example.com/link"><img src="https://example.com/example.jpg" loading="lazy" class="max-w-max max-h-80"></a></span>)
      end
    end
  end
end
