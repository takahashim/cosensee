  ...
  describe '#render' do
    context 'when anchor is not nil' do
      it 'convert ExternalLinkBracket to HTML' do
        ...
        expect(Cosensee::TailwindRenderer::ExternalLinkBracket.new(content:).render).to eq %(<span><a href="https://example.com/link" class="text-blue-500 hover:text-blue-700">foo</a></span>)
      end
    end
    ...
    context 'when anchor is nil' do
      it 'convert ExternalLinkBracket to HTML' do
        ...
        expect(Cosensee::TailwindRenderer::ExternalLinkBracket.new(content:).render).to eq %(<span><a href="https://example.com/link" class="text-blue-500 hover:text-blue-700">https://example.com/link</a></span>)
      end
    end
  end
end
